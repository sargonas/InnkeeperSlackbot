...
function formatImageResponse(object, args) {
  var card_response = '';
  //default to gold card unless regular is explicitly requsted, or unavailable
  if (array_contains(args, 'p')) {
    ...
    // ensure a gold image is available, and if not return the regular one
     if (object.imgGold === undefined) {
      card_response = 'Seems like that card doesn\'t have a golden image available, or my closest search match was off.';
      card_response.concat('\n "' + object.img + '"')
    } else {
  ...
  if (array_contains(args, 'h')) {
    card_response = 'Format your requests like "Innkeeper [C\'Thun]". \nYou can also append *-p* for a non-Gold card, *-f* to add Flavor text, and *-t* for text-only details.';
  }
